...
void main() {
  ...
  group('EventCollectionScreen', () {
    ...
    testWidgets('displays error message on load failure', (
      ...
      expect(
        find.descendant(
          of: find.byType(CustomErrorDialog),
          matching: find.text('Error loading configuration: '),
        ),
        findsOneWidget,
      );
    });
    ...
    testWidgets('shows CustomErrorDialog when viewmodel has a specific error', (
      ...
      await tester.pumpWidget(buildTestableWidget());
      await tester
          .pumpAndSettle(); // Let post frame callback run for the dialog
    ...
    testWidgets(
      'Should open AdminLoginScreen dialog after 5 taps on title when hasOrgError is true',
          (WidgetTester tester) async {

        // Configure to simulate an organization error
        when(mockCheckOrg.hasError).thenReturn(true);
        when(mockViewModel.setup()).thenAnswer((_) async => {});
        when(
          mockViewModel.viewState,
        ...
        // 3. Find the title GestureDetector by its Key
        final titleFinder = find.byKey(const Key('title_key_event_collection'));
        ...
        // 5. Verify that the Dialog and AdminLoginScreen are present
