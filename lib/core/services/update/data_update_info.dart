...
class DataUpdateInfo {
  ...
  Future<http.Response> updateSpeakers(List<Speaker> speakers) async {
    ...
    final fileUri = Uri.parse("${githubService.repo}/${speakers[0].pathUrl}?ref=${githubService.branch}");
  ...
  Future<http.Response> updateAgenda(List<Agenda> agenda) async {
    ...
    final fileUri = Uri.parse("${githubService.repo}/${agenda[0].pathUrl}?ref=${githubService.branch}");
  ...
  Future<http.Response> updateSponsors(List<Sponsor> sponsors) async {
    //List<dynamic> jsonList = await dataLoader.loadData('agenda/agenda.json');
    final fileUri = Uri.parse("${githubService.repo}/${sponsors[0].pathUrl}?ref=${githubService.branch}");
  ...
  Future<http.Response> updateEvents(List<Event> events) async {
    //List<dynamic> jsonList = await dataLoader.loadData('agenda/agenda.json');
    final fileUri = Uri.parse("${githubService.repo}/${events[0].pathUrl}?ref=${githubService.branch}");
    final sponsorInfo = base64Encode(utf8.encode(json.encode(events.map((event) => event.toJson()))));
    final body = json.encode({
      "message": "Update agendas from JSON",
      "content": sponsorInfo,
      "sha": githubService.sha
  ...
  Future<http.Response> getSha(GithubService githubService) async {
    final fileUri = Uri.parse("${githubService.repo}${githubService.repo}?ref=${githubService.branch}");

    final res = await http.get(fileUri, headers: {
      "Authorization": "Bearer ${githubService.token}",
      "Accept": "application/vnd.github.v3+json"
    });
    if(res.statusCode == 200){
